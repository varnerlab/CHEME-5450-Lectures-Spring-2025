...
Let's start with the timescales of metabolic reactions. Metabolic reactions are fast, with typical timescales on the order of milliseconds to seconds.
If we think about some metabolic reaction $r\in\mathcal{R}^{\prime}$ in a metabolic network that depends upon concentration vector $\mathbf{x}$, 
...
Next, let's consider the timescales of gene expression. Gene expression is slow, with typical timescales on the order of minutes to hours.
